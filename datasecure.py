...
# === cryptpgraphy. fernet used ===
def encrypyt_text(text,key):
        ...
        if st.button("Login"):
            if username in stored_data and stored_data[username]["password"] == hash_password(password):
                ...
                if st.session_state.failed_attempts >= 3:
                    ...
                    st.error(f"🛑 To many failed attempts. Locked for 60 seconds.")
                    st.stop()
...
# === data store section ===
    ...
    if st.session_state.authenticated_user:
        ...
        if st.button("Encrypt And Save"):
            if data and passkey:
                encrypted = encrypyt_text(data, passkey)
...
# === data retrieve data section ===
    ...
    if not st.session_state.authenticated_user:
        ...
        if not user_data:
            ...
            encrypted_input = st.textarea("Enter Encrypted Text")
